...
% =============================================================================
\title{\color{darkgray}GitHub Copilot --- Token \& Cost Optimization\\
       {\large\color{accentblue}Summary Note}}
...
\noindent\textbf{Purpose.} GitHub replaced Copilot's old premium-request system with a token-based one on 1~June 2026. The cost of a chat turn is no longer fixed: it now scales with attached context, conversation history, and the model picked. This note distils the full pack into a single read so the office can budget the new pricing without losing the productivity gains we already get from Copilot. Pin \texttt{04-cheatsheet.md} for day-to-day reference; come back here when you want the reasoning.
...
\begin{enumerate}[label=\textbf{\arabic*.}]
  \item \textbf{Model choice.} A 10--40$\times$ spread between the cheapest models (Haiku 4.5, GPT-5 mini, Gemini 3 Flash, Raptor mini) and the premium tier (Opus 4.7/4.8, GPT-5.5). For routine work the \emph{quality} gap is usually negligible.
...
\begin{itemize}
  ...
  \item \yes~Pin the cheatsheet (\texttt{04-cheatsheet.md}) next to your monitor or in an editor split.
